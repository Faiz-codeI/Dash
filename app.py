...
def style_fig(fig, height: int = 560, title: str | None = None):
    ...
    fig.update_layout(
        ...
        legend=dict(
            ...
            xanchor="left",
            x=0.01,
            font=dict(color="#e8f1f1", size=12),
            title_font=dict(color="#e8f1f1", size=12),
            bgcolor="rgba(0,0,0,0)",
        ),
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(0,0,0,0)",
        font=dict(color="#dfe6e9"),
        title=dict(x=0.01, xanchor="left", y=0.98, font=dict(size=14, color="#e8f1f1")),
    )
    fig.update_xaxes(showgrid=True, zeroline=False, gridcolor="rgba(255,255,255,0.08)")
    fig.update_yaxes(showgrid=True, zeroline=False, gridcolor="rgba(255,255,255,0.08)")
    return fig
...
def inject_css() -> None:
    st.markdown(
        ...
        html, body, [data-testid="stAppViewContainer"] {
          background: radial-gradient(120% 120% at 0% 0%, #0e0f12, #070708 60%);
        }
        [data-testid="stAppViewContainer"] { color: #dfe6e9; }
        .block-container { padding-top: 1.0rem; padding-bottom: 2rem; max-width: 1550px; }
        ...
        .dashboard-hero {
          ...
          background: linear-gradient(135deg, rgba(20,200,170,0.20), rgba(100,99,240,0.18)),
                      radial-gradient(120% 120% at 10% 0%, rgba(255,255,255,0.08), transparent 55%);
          box-shadow: 0 18px 36px rgba(0,0,0,0.45);
        }
        .hero-kicker { font-size: 0.75rem; text-transform: uppercase; letter-spacing: 1px; color: rgba(255,255,255,0.55); }
        .hero-title { font-size: 1.7rem; font-weight: 800; margin-top: 0.2rem; }
        ...
        .card {
          ...
          background: rgba(9,9,10,0.65);
          box-shadow: inset 0 0 0 1px rgba(255,255,255,0.02), 0 10px 22px rgba(0,0,0,0.35);
        }
        ...
        .dash-card::after {
          ...
          position: absolute;
          inset: 0;
          background: radial-gradient(120% 120% at 0% 0%, rgba(0,255,200,0.10), transparent 60%);
          opacity: 0.8;
          pointer-events: none;
        }
        .dash-card:hover {
        ...
        .dash-title { font-size: 0.9rem; letter-spacing: 0.4px; color: rgba(255,255,255,0.68); }
        .dash-value { font-size: 1.7rem; font-weight: 800; margin-top: 0.2rem; }
        ...
        section[data-testid="stSidebar"] {
          background: linear-gradient(180deg, #0b0c0f, #101216);
        }
        ...
        div[data-testid="stDataFrame"] div[role="row"]:nth-child(even) div[role="cell"] {
          background: rgba(14,14,16,0.95) !important;
        }
        div[data-testid="stDataFrame"] div[role="cell"] a {
          color: #7dd3fc !important;
        }

        
        /* DataFrame hard override */
        ...
        /* Sidebar neon */
        section[data-testid="stSidebar"] {
          background: linear-gradient(180deg, #0b0c0f, #0f1218);
        }
        ...
        section[data-testid="stSidebar"] .stTextInput input {
          ...
          box-shadow: 0 0 0 1px rgba(0,255,200,0.12), 0 8px 18px rgba(0,0,0,0.35);
        }
        section[data-testid="stSidebar"] .stTextInput input:focus {
          border-color: rgba(0,255,200,0.55) !important;
          box-shadow: 0 0 0 2px rgba(0,255,200,0.18);
        }
        section[data-testid="stSidebar"] .stToggle > div {
          border: 1px solid rgba(0,255,200,0.25) !important;
          box-shadow: 0 0 0 1px rgba(0,255,200,0.10);
        }
        section[data-testid="stSidebar"] .stToggle span {
          color: #cfe7e7 !important;
        }
        section[data-testid="stSidebar"] .stToggle input:checked + div {
        ...
        section[data-testid="stSidebar"] .stButton button {
          background: linear-gradient(90deg, rgba(0,255,200,0.22), rgba(0,160,255,0.22));
          color: #e8f1f1;
        ...
        section[data-testid="stSidebar"] .stSelectbox [data-baseweb="select"] svg {
          fill: #e8f1f1 !important;
        }

        
        /* Sidebar baseweb override */
        section[data-testid="stSidebar"] [data-baseweb="select"] > div {
          background: #10131a !important;
          color: #e8f1f1 !important;
          border: 1px solid rgba(0,255,200,0.35) !important;
          box-shadow: 0 0 0 1px rgba(0,255,200,0.12) !important;
        }
        ...
        .pulse-list { margin: 0.2rem 0 0.4rem 0; }
        .pulse-item {
          display: flex;
          gap: 0.5rem;
          align-items: center;
          padding: 0.28rem 0.1rem;
          color: rgba(230,240,240,0.92);
        ...
        .pulse-dot {
          ...
          border-radius: 999px;
          background: #5ef9d0;
          box-shadow: 0 0 10px rgba(94,249,208,0.45);
          flex: 0 0 8px;
        }
        .pulse-note { color: rgba(255,255,255,0.72); }
        ...
        .micro-title { font-size: 0.78rem; color: rgba(255,255,255,0.6); }
        .micro-value { font-size: 1.05rem; font-weight: 700; margin-top: 0.1rem; }
...
def render_df(df, **kwargs):
    ...
    if hasattr(df, "style"):
        style = (
            ...
            .set_table_styles([
                {"selector": "th", "props": [("background-color", "#121418"), ("color", "#cfe7e7"), ("border-color", "rgba(255,255,255,0.10)")]},
                {"selector": "thead th", "props": [("background-color", "#121418"), ("color", "#cfe7e7"), ("border-color", "rgba(255,255,255,0.10)")]},
                {"selector": "th.col_heading", "props": [("background-color", "#121418"), ("color", "#cfe7e7")]},
                {"selector": "th.index_name", "props": [("background-color", "#121418"), ("color", "#cfe7e7")]},
                {"selector": "td", "props": [("border-color", "rgba(255,255,255,0.06)")]},
            ], overwrite=False)
        )
        return st.table(style)

    return st.table(df)


def render_market_pulse_panel(items):
    st.markdown('<div class="pulse-panel">', unsafe_allow_html=True)
    st.markdown('<div class="pulse-title">🧠 Market Pulse</div>', unsafe_allow_html=True)
    if not items:
    ...
    for item in items:
        ...
        st.markdown(
            f'<div class="pulse-item"><span class="pulse-dot"></span><b>{title}:</b> {value}<span class="pulse-note">{note_suffix}</span></div>',
            unsafe_allow_html=True,
        )
...
def _mode_selector():
    st.subheader("Explore - Detail Mode")
    st.caption("Pilih mode detail untuk analisis lanjutan.")
    mode = st.radio(
        "Mode",
...
with st.sidebar:
    ...
    st.markdown("""
        <div style=\"padding:0.4rem 0 0.8rem 0;\">
          <div style=\"font-size:1.1rem; font-weight:800;\">JOURNAL</div>
          <div style=\"font-size:0.9rem; color: rgba(255,255,255,0.65);\">TRADING SAHAM</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
with tab_explore:
    ...
    <div class=\"dashboard-hero\">
      <div class=\"hero-kicker\">Explore / Dashboard</div>
      <div class=\"hero-title\">Market Pulse - Full Dashboard</div>
      <div class=\"hero-subtitle\">Ringkasan utama ditampilkan penuh di sini. Detail teknikal/fundamental ada di bagian bawah.</div>
    </div>
    """,
    unsafe_allow_html=True,
    )
    ...
    # DASHBOARD (FULL VIEW)
    # -------------------------
    if True:  # dashboard always visible
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.subheader("Dashboard Utama - Full View")
        st.markdown("<div class=\"card\" style=\"border-radius:20px; padding:0.8rem 1rem; margin-top:0.2rem;\"><b>Market Snapshot</b> - KPI + distribusi + perbandingan teknikal vs fundamental.</div>", unsafe_allow_html=True)
        st.caption("Ringkasan utama + insight visual ditampilkan penuh di satu area.")
        ...
        render_market_pulse_panel(items)
        ...
        # --- Dashboard boards (inspired by reference layout) ---
        ...
        with g1:
            ...
            fig = style_fig(fig, height=220, title="Bucket Mix")
        ...
        with g2:
            ...
            fig = style_fig(fig, height=220, title="Strong %")
            st.plotly_chart(fig, use_container_width=True)
        ...
        with g3:
            ...
            fig = style_fig(fig, height=220, title="Breadth")
            st.plotly_chart(fig, use_container_width=True)
            st.caption("")

        with g4:
            sec_mini = (
                combined.groupby("sector", dropna=False)["score_combined"]
                .mean()
                .reset_index()
            ...
            fig = style_fig(fig, height=220, title="Top Sectors")
            st.plotly_chart(fig, use_container_width=True)

        # --- Main charts row (bigger, like reference) ---
        d1, d2 = st.columns([0.58, 0.42], gap="large")
        with d1:
            view = combined.sort_values("score_combined", ascending=False).head(20).copy()
            view["rank"] = range(1, len(view) + 1)
            view = view.sort_values("rank")

            fig = go.Figure()
            fig.add_trace(go.Scatter(x=view["ticker"], y=view["score_combined"], mode="lines+markers", name="Combined"))
            fig.add_trace(go.Scatter(x=view["ticker"], y=view["health_tech"], mode="lines+markers", name="Tech"))
            fig.add_trace(go.Scatter(x=view["ticker"], y=view["fund_norm_0_100"], mode="lines+markers", name="Fund"))

            fig = style_fig(fig, height=320, title="Top 20 Scores (Combined vs Tech vs Fund)")
            fig.update_xaxes(tickangle=-35, tickfont=dict(size=9, color="#cfe7e7"))
            st.plotly_chart(fig, use_container_width=True)
        ...
        render_df(sector_board, use_container_width=True, hide_index=True)
        ...
        with left:
            ...
            render_df(rename_for_display(display), use_container_width=True, hide_index=True)
        with right:
        ...
        with st.expander("Lihat detail kandidat", expanded=False):
            ...
            render_df(rename_for_display(view[show_cols]), use_container_width=True, hide_index=True)

    st.divider()
    with st.expander("Detail Mode - Teknikal / Fundamental / Gabungan", expanded=False):
        mode = _mode_selector()
        ...
        # -------------------------
            ...
            if show_per_sector and not core_view["sector"].dropna().empty:
                ...
                for sec in sectors_sorted:
                    ...
                    with st.expander(f"{sec} — {len(sec_df):,} emiten", expanded=False):
                        ...
                        render_df(rename_for_display(df_show), use_container_width=True, hide_index=True)
            ...
            render_df(rename_for_display(view), use_container_width=True, hide_index=True)
...
with tab_compare:
    ...
    if len(picks) < 2:
        ...
        render_df(rename_for_display(comp), use_container_width=True, hide_index=True)
